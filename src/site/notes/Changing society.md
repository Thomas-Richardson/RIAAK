---
{"dg-publish":true,"permalink":"/changing-society/","tags":["#movement_strategy"],"created":"2026-01-23T12:50:24.294+00:00","updated":"2026-01-23T12:51:47.161+00:00"}
---

# One theory of change for the movement
1. Build public support for an issue
2. Use that public support to get companies to change
3. Once a few companies start to change their practices and you have public opinion, you then do the political work to translate that into policy wins that lock all of that into place.

# Related
- [[Citations/Pathways to victory - How can we end animal agriculture (Animal Ask)\|Pathways to victory - How can we end animal agriculture (Animal Ask)]]